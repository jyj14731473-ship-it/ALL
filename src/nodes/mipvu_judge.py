...
def _compact_candidate_for_llm(item: dict[str, Any]) -> dict[str, Any]:
    """Keep only fields the model must inspect for MIPVU judgment."""
    return {
        "candidate_id": str(item.get("candidate_id", "")),
        "surface_expression": str(item.get("surface_expression") or item.get("token") or ""),
        "lemma": str(item.get("lemma", "")),
        "pos": str(item.get("pos", "")),
        "context_sentence": str(item.get("context_sentence") or item.get("sentence") or ""),
        "contextual_meaning": str(item.get("contextual_meaning", "")),
        "basic_meaning": str(item.get("basic_meaning", "")),
        "basic_meaning_source": str(item.get("basic_meaning_source", "unavailable")),
    }


def _read_max_split_depth() -> int:
    raw = os.getenv("MIPVU_MAX_SPLIT_DEPTH", "2").strip()
    try:
...
def _call_mipvu_chunk(
    *,
    ...
    compact = [_compact_candidate_for_llm(item) for item in chunk]
    chunk_user_prompt = prompt_template.replace(
        "{{CANDIDATES_JSON}}",
    ...
    parsed = call_structured_chain(
        ...
        MipvuJudgmentOutput,
        errors,
        f"mipvu_judge_batch{chunk_idx}_d{depth}",
    )
    chunk_judgments = parsed.get("judgments", []) if isinstance(parsed, dict) else []
    if chunk_judgments or len(chunk) <= 1 or not os.getenv("OPENAI_API_KEY", "").strip():
        return [item for item in chunk_judgments if isinstance(item, dict)]

    max_depth = _read_max_split_depth()
    if depth >= max_depth:
        errors.append(f"[mipvu_judge_batch{chunk_idx}] 빈 결과를 받았지만 max split depth에 도달했습니다.")
        return []

    midpoint = len(chunk) // 2
    left = _call_mipvu_chunk(
...
def mipvu_judge_node(state: AnnotationState) -> dict[str, Any]:
    ...
    prompt_template = load_prompt_text(prompt_dir, "mipvu_judge.md")
    if not prompt_template:
        prompt_template = "후보별 MIPVU 판정을 JSON으로 반환하세요.\n\n{{CANDIDATES_JSON}}"
    ...
    for idx, item in enumerate(judgments if isinstance(judgments, list) else [], start=1):
        ...
        judgment["candidate_id"] = str(judgment.get("candidate_id") or source.get("candidate_id", f"C{idx:03d}"))
        judgment["sentence_id"] = source.get("sentence_id", f"S{idx:03d}")
        judgment["token"] = judgment.get("token") or judgment.get("surface_expression") or source.get("surface_expression", "")
        judgment["lemma"] = source.get("lemma", "")
        judgment["pos"] = source.get("pos", "")
        judgment["context_sentence"] = source.get("sentence", source.get("context_sentence", ""))
        judgment["contextual_meaning"] = source.get("contextual_meaning", "")
        judgment["basic_meaning"] = source.get("basic_meaning", "")
        judgment["basic_meaning_source"] = source.get("basic_meaning_source", "unavailable")
        judgment["meaning_contrast"] = source.get("meaning_contrast", "")
        judgment["distinctness"] = source.get("distinctness", False)
        judgment["comparison_possible"] = source.get("comparison_possible", False)
        judgment["similarity"] = source.get("similarity", False)
        judgment["pre_mipvu_judgment"] = source.get("pre_mipvu_judgment", "uncertain")
        judgment["pre_mipvu_confidence"] = source.get("pre_mipvu_confidence", 0.0)
